...
double multiplyParallel(int size, double **numArray1, double **numArray2, double **resultArray);
...
int main()
{
	...
	for (int i = 200; i < 2001; i = i + 200) {
		...
		for (int step = 0; step<stepCount; step += 1) {
			...
			double parallelTime = multiplyParallel(size, initial2DArray1, initial2DArray2, resultArray);

			sum += parallelTime;
			sqrSum += parallelTime*parallelTime;
